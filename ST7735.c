...
// Output: none
// Must be less than or equal to 128 pixels wide by 160 pixels high
void ST7735_DrawBitmap_Mirror(int16_t x, int16_t y, const uint16_t *image, int16_t w, int16_t h){
  int16_t skipC = 0;                      // non-zero if columns need to be skipped due to clipping
  int16_t originalWidth = w;              // save this value; even if not all columns fit on the screen, the image is still this width in ROM
  int i = 0;
...
uint32_t ST7735_DrawString_size(uint16_t x, uint16_t y, char *pt, int16_t textColor,uint8_t size){
  uint32_t count = 0;
  if(y>15) return 0;
	x = x-size;
	ST7735_DrawCharS(x*6, y*10, 32, textColor, ST7735_BLACK, size);
	x = x+size;
  while(*pt){
		ST7735_DrawCharS(x*6, y*10, 32, textColor, ST7735_BLACK, size);
...
void ST7735_OutUDec(uint32_t n,uint8_t size){
  ...
  ST7735_DrawString_size(StX,StY,Message,StTextColor,size);
