...
wait    LDR R3, =SSI0_SR_R
		LDR R1, [R3]			;read SSIO_SR_R
		...
		LDR R1, =SSI0_DR_R		
		STR R0, [R1]			;store command to SSIO_DR_R
wait2	LDR R2, [R3]			;read SSIO_SR_R
		AND R2, R2, #0x10
...
loop    LDR R1, =SSI0_SR_R
		LDR R1, [R1]	;load ssio source data
		AND R1, R1, #1	;isolate bit 0
		CMP R1, #0		
		BEQ loop		;if low loop
		LDR R2, =DC
		LDR R3, [R2]
		ORR R3, R3, #80	;set port a bit
		STR R3,[R2]	
		LDR R1, =SSI0_DR_R
		STR R0, [R1]
